...
public class AlquilarAtaStepdefinition {
    ...
    @Dado("que un cliente quiere alquilar un lugar de estadia para vacaciones")
    public void queUnClienteQuiereAlquilarUnLugarDeEstadiaParaVacaciones() {
        OnStage.theActorInTheSpotlight().attemptsTo(Consulta.rentaVacation());

    }

    @Cuando("consulta el lugar de destino")
    public void consultaElLugarDeDestino() {
        OnStage.theActorInTheSpotlight().attemptsTo(Consulta.opcionApar());
    }

    @Entonces("debe poder ver los resultados de los lugares")
    public void debePoderVerLosResultadosDeLosLugares() {
        OnStage.theActorInTheSpotlight().should(GivenWhenThen.seeThat(Resultados.Consultas(ConsultaPage.LBL_RESULTADOS_UNO),
                ...
                .orComplainWith(ExceptionDeResultados.class, MensajesExcepciones.LBL_ERROR_RESULTADOS_CINCO));
    }
}
